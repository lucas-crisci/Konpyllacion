...
listDecVariable : listDecVar POINT_VIRGULE {$$=$1;} | {$$=NULL;} ;
...
decFct : IDENTIF PARENTHESE_OUVRANTE listArg PARENTHESE_FERMANTE listDecVariable instrBloc {$$ = cree_n_dec_fonc($1, $3, $5, $6);};
listArg :  listDecVar  {$$=$1;} | {$$=NULL;};
...
appelFct : IDENTIF PARENTHESE_OUVRANTE listeExp PARENTHESE_FERMANTE {$$ = cree_n_appel($1, $3);};

listeExp : expression listeExpBis {$$=cree_n_l_exp($1,$2);}
    |			{$$=NULL;}
    ;

listeExpBis : VIRGULE expression listeExpBis {$$=cree_n_l_exp($2,$3);}
      | 		{$$=NULL;}
      ;
